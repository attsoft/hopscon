...
function ddwc_order_driver_details( $order ) {
    ...
    if ( get_user_meta( $driver_id, 'ddwc_driver_licence_picture', true ) ) {
        $driver_licence_pic = get_user_meta( $driver_id, 'ddwc_driver_licence_picture', true );
    }
    if ( get_user_meta( $driver_id, 'ddwc_driver_id_picture', true ) ) {
        $driver_id_pic = get_user_meta( $driver_id, 'ddwc_driver_id_picture', true );
    }
    ...
    if ( '-1' !== $driver_id && '' !== $driver_id ) {
        ...
        if ( get_user_meta( $driver_id, 'ddwc_driver_licence_picture', true ) ) {
            $driver_licence_pic = get_user_meta( $driver_id, 'ddwc_driver_licence_picture', true );
            $string    .= '<a href="' . $driver_licence_pic['url'] . '"><img src="' . $driver_licence_pic['url'] . '" alt="' . $driver_meta->user_firstname . ' ' . $driver_meta->user_lastname . '" /></a>';
        } else {
            // Do nothing.
        }
        if ( get_user_meta( $driver_id, 'ddwc_driver_id_picture', true ) ) {
            $driver_id_pic = get_user_meta( $driver_id, 'ddwc_driver_id_picture', true );
            $string    .= '<a href="' . $driver_id_pic['url'] . '"><img src="' . $driver_id_pic['url'] . '" alt="' . $driver_meta->user_firstname . ' ' . $driver_meta->user_lastname . '" /></a>';
        } else {
            // Do nothing.
        }
        // Driver name.
        ...
        if ( 'completed' == $order_status ) {
            // Star ratings.
            $string .= '<h5 class="rate_delivery">' . esc_attr__( 'Rate Delivery', 'ddwc' ) . '</h5>';
            $string .= '<br /><select class="driver-rating" id="rating_' . $order_id . '" data-id="rating_' . $order_id . '"><option value=""></option><option value="1">1</option><option value="2">2</option><option value="3">3</option><option value="4">4</option><option value="5">5</option></select>';
            ...
            if ( 'no' !== get_option( 'ddwc_settings_driver_phone_number' ) ) {
                ...
                if ( $driver_number ) {
                    $string .= '<a href="tel:' . $driver_number . '" class="button ddwc-button customer">' . esc_attr__( 'Call Driver', 'ddwc' ) . '</a> ';
                }
            } else {
                // Do nothing.
            }
        ...
        if ( get_user_meta( $driver_id, 'ddwc_driver_vehicle_color', true ) ) {
            // Color name.
            $color_name = esc_attr__( 'Vehicle Color', 'ddwc' );
        ...
        if ( get_user_meta( $driver_id, 'ddwc_driver_vehicle_model', true ) ) {
            // Model name.
            $model_name = esc_attr__( 'Vehicle Model', 'ddwc' );
            if ( '' != get_user_meta( $driver_id, 'ddwc_driver_transportation_type', TRUE ) ) {
        ...
        if ( get_user_meta( $driver_id, 'ddwc_driver_id_type', true ) ) {
            $string .= '<td>' . esc_attr__( 'ID Type', 'ddwc' ) . '<br /><strong>' . get_user_meta( $driver_id, 'ddwc_driver_id_type', true ) . '</strong></td>';
        }
        // ID number
        if ( get_user_meta( $driver_id, 'ddwc_driver_id_number', true ) ) {
            $string .= '<td>' . esc_attr__( 'ID Number', 'ddwc' ) . '<br /><strong>' . get_user_meta( $driver_id, 'ddwc_driver_id_number', true ) . '</strong></td>';
        }
        // Driver License plate.
        if ( get_user_meta( $driver_id, 'ddwc_driver_license_plate', true ) ) {
            $string .= '<td>' . esc_attr__( 'License Plate', 'ddwc' ) . '<br /><strong>' . get_user_meta( $driver_id, 'ddwc_driver_license_plate', true ) . '</strong></td>';
        }
...
function ddwc_save_custom_profile_fields( $user_id ) {
    ...
    $id_number       = filter_input( INPUT_POST, 'ddwc_driver_id_number' );
    $id_type       = filter_input( INPUT_POST, 'ddwc_driver_id_type' );
    ...
    $remove_license_picture     = filter_input( INPUT_POST, 'ddwc_driver_licence_picture' );
    $remove_id_picture     = filter_input( INPUT_POST, 'ddwc_driver_id_picture' );
    ...
    if ( in_array( 'driver', (array) $user->roles ) ) {
        ...
        // Update ID number.
        if ( isset( $id_number ) ) {
            update_user_meta( $user_id, 'ddwc_driver_id_number', esc_html( $id_number ) );
        }
        // Update ID type.
        if ( isset( $id_type ) ) {
            update_user_meta( $user_id, 'ddwc_driver_id_type', esc_html( $id_type ) );
        }
        ...
        if ( isset( $remove_license_picture ) ) {
            update_user_meta( $user_id, 'ddwc_driver_licence_picture', '' );
        }
        if ( isset( $remove_id_picture ) ) {
            update_user_meta( $user_id, 'ddwc_driver_id_picture', '' );
        }
        // If no new files are uploaded, return.
        if ( ! isset( $_FILES ) || empty( $_FILES ) || ! isset( $_FILES['ddwc_driver_picture']  )  || ! isset( $_FILES['ddwc_driver_licence_picture']  ) || ! isset( $_FILES['ddwc_driver_id_picture']  ) ) {
            return;
        }
        ...
        $ddwc_driver_licence_picture = wp_handle_upload( $_FILES['ddwc_driver_licence_picture'], array( 'test_form' => false, 'mimes' => array( 'jpg' => 'image/jpeg', 'gif' => 'image/gif', 'png' => 'image/png', 'jpeg' => 'image/jpeg' ) ) );

        $ddwc_driver_id_picture = wp_handle_upload( $_FILES['ddwc_driver_id_picture'], array( 'test_form' => false, 'mimes' => array( 'jpg' => 'image/jpeg', 'gif' => 'image/gif', 'png' => 'image/png', 'jpeg' => 'image/jpeg' ) ) );
        ...
        if ( isset( $ddwc_driver_licence_picture['file'] ) ) {
            // Update driver picture meta.
            update_user_meta( $user_id, 'ddwc_driver_licence_picture', $ddwc_driver_licence_picture, get_user_meta( $user_id, 'ddwc_driver_licence_picture', true ) );

            $filename   = $ddwc_driver_licence_picture['file'];
            $title      = explode( '.', basename( $filename ) );
            $attachment = array(
                'guid'           => $ddwc_driver_licence_picture['url'],
                'post_mime_type' => $ddwc_driver_licence_picture['type'],
                'post_title'     => implode( '.', $title ),
                'post_content'   => '',
                'post_status'    => 'inherit'
            );
        ...
        if ( isset( $ddwc_driver_id_picture['file'] ) ) {
            // Update driver picture meta.
            update_user_meta( $user_id, 'ddwc_driver_id_picture', $ddwc_driver_id_picture, get_user_meta( $user_id, 'ddwc_driver_id_picture', true ) );

            $filename   = $ddwc_driver_id_picture['file'];
            $title      = explode( '.', basename( $filename ) );
            $attachment = array(
                'guid'           => $ddwc_driver_id_picture['url'],
                'post_mime_type' => $ddwc_driver_id_picture['type'],
                'post_title'     => implode( '.', $title ),
                'post_content'   => '',
                'post_status'    => 'inherit'
            );
...
function ddwc_add_profile_options( $profileuser ) {
    // Get driver picture.
    $ddwc_driver_picture_id = get_user_meta( $profileuser->ID, 'ddwc_driver_picture', true );
    $ddwc_driver_picture_attachment = wp_get_attachment_url($ddwc_driver_picture_id);

    if (empty($ddwc_driver_picture_attachment)) {
    	$ddwc_driver_picture = 	$ddwc_driver_picture_id;
    	$ddwc_driver_picture = $ddwc_driver_picture['url'];
    }else{
		$ddwc_driver_picture = $ddwc_driver_picture_attachment;   	
    }

    // Get driver licence picture.
    $ddwc_driver_licence_picture_id = get_user_meta( $profileuser->ID, 'ddwc_driver_licence_picture', true );
    $ddwc_driver_licence_picture_attachment = wp_get_attachment_url($ddwc_driver_licence_picture_id);

    if (empty($ddwc_driver_licence_picture_attachment)) {
    	$ddwc_driver_licence_picture = 	$ddwc_driver_licence_picture_id;
    	$ddwc_driver_licence_picture = $ddwc_driver_licence_picture_id['url'];
    }else{
		$ddwc_driver_licence_picture = $ddwc_driver_licence_picture_attachment;   	
    }

    // Get driver id picture
    $ddwc_driver_id_picture_id = get_user_meta( $profileuser->ID, 'ddwc_driver_id_picture', true );
    $ddwc_driver_id_picture_attachment = wp_get_attachment_url($ddwc_driver_id_picture_id);

    if (empty($ddwc_driver_licence_picture_attachment)) {
    	$ddwc_driver_id_picture = 	$ddwc_driver_id_picture_id;
    	$ddwc_driver_id_picture = $ddwc_driver_id_picture_id['url'];
    }else{
		$ddwc_driver_id_picture = $ddwc_driver_id_picture_attachment;   	
    }
    ...
    ?>
        ...
        <tr>
            <th scope="row"><?php esc_html_e( 'Driver License Photo', 'ddwc' ); ?></th>
            <td class="ddwc-driver-licence-picture">
                <?php if ( get_user_meta( $profileuser->ID, 'ddwc_driver_licence_picture', true ) ) { ?>
                <div class="ddwc-driver-licence-picture"><!-- /.ddwc-driver-licence-picture -->
                  <?php
                      if ( ! isset( $ddwc_driver_licence_picture['error'] ) ) {
                          if ( ! empty( $ddwc_driver_licence_picture ) ) {
                              // $ddwc_driver_licence_picture = $ddwc_driver_licence_picture['url'];
                              echo '<a href="' . $ddwc_driver_licence_picture . '" target="_blank"><img src="' . $ddwc_driver_licence_picture . '" width="100" height="100" class="ddwc-driver-licence-picture" /></a><br />';
                          }
                      } else {
                          $ddwc_driver_licence_picture = $ddwc_driver_licence_picture['error'];
                          echo $ddwc_driver_licence_picture. '<br />';
                      }
                  ?>
                  <button class="ddwc-remove-driver-licence-picture" name="ddwc_driver_licence_picture"><?php esc_html_e( 'x', 'ddwc' ); ?></button>
                </div><!-- /.ddwc-driver-licence-picture -->
                <?php } ?>
                <input type="file" name="ddwc_driver_licence_picture" value="" />
            </td>
        </tr>
        <tr>
            <th scope="row"><?php esc_html_e( 'Driver ID Photo', 'ddwc' ); ?></th>
            <td class="ddwc-driver-id-picture">
                <?php if ( get_user_meta( $profileuser->ID, 'ddwc_driver_id_picture', true ) ) { ?>
                <div class="ddwc-driver-id-picture"><!-- /.ddwc-driver-licence-picture -->
                  <?php
                      if ( ! isset( $ddwc_driver_id_picture['error'] ) ) {
                          if ( ! empty( $ddwc_driver_id_picture ) ) {
                              // $ddwc_driver_id_picture = $ddwc_driver_id_picture['url'];
                              echo '<a href="' . $ddwc_driver_id_picture . '" target="_blank"><img src="' . $ddwc_driver_id_picture . '" width="100" height="100" class="ddwc-driver-id-picture" /></a><br />';
                          }
                      } else {
                          $ddwc_driver_id_picture = $ddwc_driver_id_picture['error'];
                          echo $ddwc_driver_id_picture. '<br />';
                      }
                  ?>
                  <button class="ddwc-remove-driver-id-picture" name="ddwc_driver_id_picture"><?php esc_html_e( 'x', 'ddwc' ); ?></button>
                </div><!-- /.ddwc-driver-licence-picture -->
                <?php } ?>
                <input type="file" name="ddwc_driver_id_picture" value="" />
            </td>
        </tr>
        <tr>
            <th scope="row"><?php esc_html_e( 'ID Number', 'ddwc' ); ?></th>
            <td>
                <input class="regular-text" type="text" name="ddwc_driver_id_number" value="<?php echo esc_html( get_user_meta( $profileuser->ID, 'ddwc_driver_id_number', true ) ); ?>" />
            </td>
        </tr>
        <tr>
        ...
        <tr>
            <th scope="row"><?php esc_html_e( 'Id Type', 'ddwc' ); ?></th>
            <td>
            <?php
                // Transportation types.
                $id_types = apply_filters( 'ddwc_woocommerce_edit_account_id_types', array( esc_attr__( 'Passport', 'ddwc' ), esc_attr__( 'National Id', 'ddwc' ), esc_attr__( 'Health Card', 'ddwc' ), esc_attr__( 'RAMQ', 'ddwc' ) ) );
                // Loop through types.
                if ( $id_types ) {
                    printf( '<select name="ddwc_driver_id_type" id="ddwc_driver_id_type" name="ddwc_driver_id_type">', get_user_meta( $user->ID, 'ddwc_driver_id_type', TRUE ) );
                    echo '<option value="">--</option>';
                    foreach ( $id_types as $type ) {
                        if ( $type != get_user_meta( $user->ID, 'ddwc_driver_id_type', TRUE ) ) {
                            $imagesizeinfo = '';
                        } else {
        ...
        <tr>
            ...
            <?php
                // Transportation types.
                $transportation_types = apply_filters( 'ddwc_woocommerce_edit_account_transportation_types', array( esc_attr__( 'Bicycle', 'ddwc' ), esc_attr__( 'Motorcycle', 'ddwc' ), esc_attr__( 'Car', 'ddwc' ), esc_attr__( 'SUV', 'ddwc' ), esc_attr__( 'Truck', 'ddwc' ) ) );
                // Loop through types.
...
function ddwc_add_to_edit_account_form() {
    ...
    if ( isset( $id_number ) ) {
        update_user_meta( $user->ID, 'ddwc_driver_id_number', $id_number );
    }
    if ( isset( $id_type ) ) {
        update_user_meta( $user->ID, 'ddwc_driver_id_type', $id_type );
    }
    ?>
    <?php
    /**
    ...
    if ( in_array( 'driver', (array) $user->roles ) ) { ?>
    <fieldset class="driver_ac_block">
        ...
        <p class="woocommerce-form-row woocommerce-form-row--wide form-row form-row-wide">
            ...
            <div class="ddwc-driver-picture">
              <?php
              $ddwc_driver_picture_id = get_user_meta(  $user->ID, 'ddwc_driver_picture', true );
			    $ddwc_driver_picture_attachment = wp_get_attachment_url($ddwc_driver_picture_id);
        ...
        <p class="woocommerce-form-row woocommerce-form-row--wide form-row form-row-wide">
            <label for="reg_ddwc_driver_licence_picture"><?php esc_html_e( 'Driver License Picture', 'ddwc' ); ?></label>
            <?php if ( get_user_meta( $user->ID, 'ddwc_driver_licence_picture', true ) ) { ?>
            <div class="ddwc_driver_licence_picture"><!-- /.ddwc-driver-licence-picture -->
              <?php
              $ddwc_driver_licence_picture_id = get_user_meta( $user->ID, 'ddwc_driver_licence_picture', true );
			    $ddwc_driver_licence_picture_attachment = wp_get_attachment_url($ddwc_driver_licence_picture_id);
              ...
              if ( ! isset( $ddwc_driver_licence_picture['error'] ) ) {
                  if ( ! empty( $ddwc_driver_licence_picture ) ) {
                      // $ddwc_driver_licence_picture = $ddwc_driver_licence_picture['url'];
                      echo '<a href="' . esc_html( $ddwc_driver_licence_picture ) . '" target="_blank"><img src="' . esc_html( $ddwc_driver_licence_picture ) . '" width="100" height="100" class="ddwc_driver_licence_picture" /></a><br />';
                  }
              } else {
                  $ddwc_driver_licence_picture = $ddwc_driver_licence_picture['error'];
                  echo esc_html( $ddwc_driver_licence_picture ) . '<br />';
              }
              ?>
              <button class="ddwc_driver_licence_picture" name="ddwc_driver_licence_picture"><?php esc_html_e( 'x', 'ddwc' ); ?></button>
            </div><!-- /.ddwc-driver-licence-picture -->
            <?php } ?>
            <input type="file" name="ddwc_driver_licence_picture" id="reg_ddwc_driver_licence_picture" value="" />
        </p>
        <p class="woocommerce-form-row woocommerce-form-row--wide form-row form-row-wide">
            <label for="reg_ddwc_driver_id_picture"><?php esc_html_e( 'Driver ID Picture', 'ddwc' ); ?></label>
            <?php if ( get_user_meta( $user->ID, 'ddwc_driver_id_picture', true ) ) { ?>
            <div class="ddwc_driver_id_picture"><!-- /.ddwc-driver-licence-picture -->
              <?php
              $ddwc_driver_id_picture_id = get_user_meta( $user->ID, 'ddwc_driver_id_picture', true );
			    $ddwc_driver_id_picture_attachment = wp_get_attachment_url($ddwc_driver_id_picture_id);
              ...
              if ( ! isset( $ddwc_driver_id_picture['error'] ) ) {
                  if ( ! empty( $ddwc_driver_id_picture ) ) {
                      // $ddwc_driver_id_picture = $ddwc_driver_id_picture['url'];
                      echo '<a href="' . esc_html( $ddwc_driver_id_picture ) . '" target="_blank"><img src="' . esc_html( $ddwc_driver_id_picture ) . '" width="100" height="100" class="ddwc_driver_id_picture" /></a><br />';
                  }
              } else {
                  $ddwc_driver_id_picture = $ddwc_driver_id_picture['error'];
                  echo esc_html( $ddwc_driver_id_picture ) . '<br />';
              }
              ?>
              <button class="ddwc_driver_id_picture" name="ddwc_driver_id_picture"><?php esc_html_e( 'x', 'ddwc' ); ?></button>
            </div><!-- /.ddwc-driver-licence-picture -->
            <?php } ?>
            <input type="file" name="ddwc_driver_id_picture" id="ddwc_driver_id_picture" value="" />
        </p>
        <p class="woocommerce-form-row woocommerce-form-row--wide form-row form-row-wide">
            <label for="reg_ddwc_driver_id_type"><?php esc_html_e( 'ID Type', 'ddwc' ); ?></label>
            <select class="input-text" name="ddwc_driver_id_type" id="reg_ddwc_driver_id_type" class="woocommerce-Input woocommerce-Input--idtype input-text" value="<?php echo get_user_meta( $user->ID, 'ddwc_driver_id_type', true ); ?>">
                <option value="">Select ID Type</option>
                <option value="Passport" <?php if (get_user_meta( $user->ID, 'ddwc_driver_id_type', true ) == 'Passport') {
                   echo "selected";
                } ?> >Passport</option>
                <option value="National ID" <?php if (get_user_meta( $user->ID, 'ddwc_driver_id_type', true ) == 'National ID') {
                   echo "selected";
                } ?>>National ID</option>
                <option value="Health Card" <?php if (get_user_meta( $user->ID, 'ddwc_driver_id_type', true ) == 'Health Card') {
                   echo "selected";
                } ?>>Health Card</option>
                <option value="RAMQ" <?php if (get_user_meta( $user->ID, 'ddwc_driver_id_type', true ) == 'RAMQ') {
                   echo "selected";
                } ?>>RAMQ</option>
            </select>
        </p>
        <p class="woocommerce-form-row woocommerce-form-row--wide form-row form-row-wide">
            <label for="reg_ddwc_driver_id_number"><?php esc_html_e( 'ID Number', 'ddwc' ); ?></label>
            <input type="text" class="input-text" name="ddwc_driver_id_number" id="reg_ddwc_driver_id_number" value="<?php echo get_user_meta( $user->ID, 'ddwc_driver_id_number', true ); ?>" />
        </p>
        <p class="woocommerce-form-row woocommerce-form-row--wide form-row form-row-wide">
